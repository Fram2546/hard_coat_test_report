...
<?php 
    ...
    if (isset($_POST['update'])) {
       ...
       $stmt = $conn->prepare("UPDATE cx SET cx = :cx WHERE id = :id");
...
<body>
    <div class="container mt-5">
        <h1>Edit Data</h1>
        <hr>
        <form action="edit_cx.php" method="post" enctype="multipart/form-data">
            <?php
                if (isset($_GET['id'])) {
                        $id = $_GET['id'];
                        $stmt = $conn->query("SELECT * FROM cx WHERE id = $id");
                        $stmt->execute();
                        $data = $stmt->fetch();
                }
            ?>
                <div class="mb-3">
                    ...
                    <input type="text" value="<?php echo $data['cx']; ?>" required class="form-control" name="cx" >
                </div>
